...
class Summarization:
    ...
    def get_summary(self, post_df_dict, temp_llm):
        for coin, post_df in post_df_dict.items():
            filtered_descriptions = self.summarize_data(post_df, coin)

            filtered_descriptions.to_csv(f"/Users/krishnayadav/Downloads/coin_data/summarized_data/{temp_llm}.csv")
...
if __name__ == "__main__":
    llm_list = ["mistral", "phi3", "llama2-uncensored"]
    for temp_llm in llm_list:
        print(temp_llm)
        summarize = Summarization(temp_llm)
        post_directory = '/Users/krishnayadav/Downloads/ML_train/train_3_15.csv'
        post_df_dict = read_files(post_directory)
        summarize.get_summary(post_df_dict, temp_llm)
